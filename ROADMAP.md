...
all three browser engines. Only D4 (code editor with syntax highlighting and
file associations) remains before step 16b's exit criteria are fully met; it
re-exercises the manifest pattern that step 17 has to carry.
...
2. **How far the app suite grows** (D-area): the current answer is "fill the
   obvious holes, then stop" — markdown preview (✅ shipped, step 15), PDF and
   small utilities (✅ both shipped, step 16b), and a code editor (D4,
   step 16b, still open), plus Notes' inline WYSIWYG (D9) as a further
   refinement. A spreadsheet, a full-featured drawing app (Paint stays a
   scratch canvas, not an image editor), or an archive manager would be a new
