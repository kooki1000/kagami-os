...
one ends. Settings offers three accents, three wallpapers, dock size and
position, and nothing else; the file-association overrides it stores have no
UI at all. Closing that gap is area **U** (§3) and step 15 (§4), and it is
...
| U3  | **More curated directions** — grow beyond Lagoon/Iris/Meadow                                                                                                                                                                  | S    | Cheap; presets remain the recommended path even after U2                                                                                                                                                                |
...
| R3  | **Dependency policy blocks a needed library.** `minimumReleaseAge: 10080` + `blockExoticSubdeps` already blocked `idb`; CodeMirror 6 (D4) and pdf.js (D6) are the next asks                                                       | Med  | Med  | Spike both in **week 1 of step 14**, well before step 16 depends on the answer; prefer zero-dep or vendorable options; budget for hand-rolling thin layers (the raw-IDB adapter proves this is viable)                                                                                                        |
...
1. **Add a LICENSE** (§6.3). The repository is public without one, so today
   ...
   nothing.
2. **Run the dependency spike** (R3) — try installing CodeMirror 6 and
   pdf.js under the workspace policy. Costs an hour; shapes step 16b.
