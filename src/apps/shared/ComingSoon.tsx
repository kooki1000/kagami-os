...
export function ComingSoon({ appName, phase, icon: Icon, blurb }: ComingSoonProps) {
  return (
    <div className="flex h-full flex-col items-center justify-center gap-3 p-8 text-center select-none">
      ...
      <div className="text-[calc(15px*var(--ui-scale))] font-semibold text-ink">{appName}</div>
      <p className="max-w-72 text-13/relaxed text-ink-2">{blurb}</p>
      <span className="mt-1 rounded-full bg-ph px-3 py-1 text-11 font-medium text-ink-2">
